...
import { GraduationCap, Clock, Calendar, QrCode, Sparkles, Loader2 } from 'lucide-react';
...
const DisplayMode: React.FC = () => {
  ...
  const fetchKioskNotices = React.useCallback(async () => {
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col justify-between overflow-y-auto lg:overflow-hidden p-4 sm:p-6 select-none">
      
      {/* Top Banner (Header) */}
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 pb-4 sm:pb-5 gap-3 shrink-0">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <div className="p-2 sm:p-2.5 bg-indigo-600 rounded-xl text-white shadow-lg shadow-indigo-500/20">
            <GraduationCap className="w-6 h-6 sm:w-8 sm:h-8 animate-pulse" />
          </div>
          <div>
            <span className="text-xl sm:text-2xl font-black tracking-tight text-white block">DIGINOTICE AI</span>
            <span className="text-[9px] sm:text-[10px] font-extrabold text-slate-500 uppercase tracking-widest block">Digital Signage Network</span>
          </div>
        </div>

        {/* Live Clock Panel */}
        <div className="flex items-center gap-4 sm:gap-6 text-slate-400">
          <div className="flex items-center gap-2 border-r border-slate-800 pr-4 sm:pr-6 text-xs sm:text-sm font-bold uppercase tracking-wide">
            <Calendar className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-500" />
            <span>{currentTime.toLocaleDateString(undefined, { dateStyle: 'medium' })}</span>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2 text-lg sm:text-2xl font-black text-white font-mono">
            <Clock className="w-4 h-4 sm:w-6 sm:h-6 text-indigo-500" />
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center py-12">
          ...
          <p className="text-slate-500 text-xs sm:text-sm mt-3 font-semibold tracking-wide">Connecting to board display feeds...</p>
        </div>
      ) : notices.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6 sm:p-12">
          <div className="p-4 sm:p-5 bg-slate-900 border border-slate-800 text-slate-500 rounded-full mb-4 sm:mb-6">
            <QrCode className="w-12 h-12 sm:w-16 sm:h-16" />
          </div>
          <h2 className="text-2xl sm:text-3xl font-black tracking-tight">No active announcements</h2>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-6 sm:gap-8 py-6 sm:py-8 items-stretch">
          
          {/* Left 3 cols: Notice Content Display */}
          <div className="lg:col-span-3 bg-slate-900/40 border border-slate-800/80 rounded-3xl p-5 sm:p-8 flex flex-col justify-between relative overflow-hidden backdrop-blur-xl">
            
            {/* Background design accents */}
            <div className="absolute right-0 top-0 translate-x-20 -translate-y-20 w-72 sm:w-96 h-72 sm:h-96 bg-indigo-500/5 rounded-full pointer-events-none filter blur-2xl" />

            <div>
              {/* Category & priority badges */}
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                <span className={`px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-black tracking-wider uppercase ${getPriorityColor(activeNotice.priority)}`}>
                  {activeNotice.priority} Notice
                </span>
                <span className="text-slate-500 font-extrabold text-xs sm:text-sm uppercase tracking-widest">
                  Category: {activeNotice.category}
                </span>
                {activeNotice.department && (
                  <span className="bg-slate-800 text-indigo-400 border border-indigo-900/30 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase">
              ...
              {/* Title */}
              <h2 className="text-2xl sm:text-3xl md:text-5xl font-black tracking-tight text-white leading-tight mt-4 sm:mt-6">
                {activeNotice.title}
              </h2>

              {/* Content body */}
              <div className="text-slate-300 text-sm sm:text-base md:text-lg mt-4 sm:mt-8 leading-relaxed font-semibold max-h-[220px] sm:max-h-[300px] overflow-hidden relative">
                <p className="whitespace-pre-line">{activeNotice.content}</p>
                {/* Fade effect at bottom if long */}
                <div className="absolute bottom-0 left-0 w-full h-10 sm:h-12 bg-gradient-to-t from-slate-950 to-transparent pointer-events-none" />
              </div>
            </div>

            {/* AI Summary Banner at bottom */}
            {activeNotice.summary && (
              <div className="bg-indigo-950/40 border border-indigo-800/40 p-5 rounded-2xl flex items-start gap-4 shadow-lg shadow-black/10 mt-6">
